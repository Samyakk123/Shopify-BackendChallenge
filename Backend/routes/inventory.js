...
router.get('/filter/:inventory', (req, res) => {
  ...
  if(temp.description !== '') query.$and.push({description : {$all : temp.description} })
  if(temp.quantity !== '0') query.$and.push({quantity: { $gt: temp.quantity} })
  if(temp.brand !== '') query.$and.push({brand : {$all : temp.brand} })
  if(temp.tags !== '') query.$and.push({tags : {$all : temp.tags.split("&")} })

  //query.$and.push({$and: subQuery})
  //query.$and.push(subQuery)
  console.log("OVA BEFORE: ", query)
  console.log("OVA: ", query.$and)


  inventorySchema.find(query).then(result => {
